...
REM Companyname: HSH
REM Productname: Multiprofile starter for 64Gram/Kotatogram/Telegram/iMe
...
title Multiprofile starter for 64Gram/Kotatogram/Telegram/iMe/iMe
color 1f

if not exist %0.lock goto start
echo Process aborted: this program is already running
echo Close all 64Gram/Kotatogram/Telegram/iMe instances
...
%MYFILES%\nircmdc.exe win show ititle "Multiprofile starter for 64Gram/Kotatogram/Telegram/iMe"
echo ---------------------------
echo ^<^<^<        Done!        ^>^>^>
echo ---------------------------
echo.
...
SET /P ask="64Gram - press 1, Kotatogram - press 3, Telegram - press 5, iMe - press 7>"
if "%ask%" NEQ "1" if "%ask%" NEQ "3" if "%ask%" NEQ "5" if "%ask%" NEQ "7" goto askingtdata
...
echo where Telegram.exe, 64Gram.exe, Kotatogram.exe or iMe.exe exists.
echo.
